...
describe MiniGit do
  ...
  describe '#[]' do
    it 'returns nil if the passed in attribute has no value' do
      MiniGit::Executor.any_instance.expects(:run).with('git', 'config', 'foo.bar').
        at_least_once.
    ...
    it 'returns a stripped configuration value if it exists' do
      MiniGit::Executor.any_instance.expects(:run).with('git', 'config', 'foo.baz').
        at_least_once.
        returns("whatever\n")
  ...
  describe '#[]=' do
    it 'assigns value to a git config attribute' do
      MiniGit::Executor.any_instance.expects(:run).with('git', 'config', 'bar.baz', 'foo')
      git['bar.baz'] = 'foo'
    end
  end
