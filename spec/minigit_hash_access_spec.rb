...
describe MiniGit do
  ...
  describe '#[]' do
    it 'returns nil if the passed in attribute has no value' do
      MiniGit::Spec::EXECUTOR.expects(:call).with('git', 'config', 'foo.bar').
        at_least_once.
    ...
    it 'returns a stripped configuration value if it exists' do
      MiniGit::Spec::EXECUTOR.expects(:call).with('git', 'config', 'foo.baz').
        at_least_once.
        returns("whatever\n")
  ...
  describe '#[]=' do
    it 'assigns value to a git config attribute' do
      MiniGit::Spec::EXECUTOR.expects(:call).with('git', 'config', 'bar.baz', 'foo')
      git['bar.baz'] = 'foo'
    end
  end
